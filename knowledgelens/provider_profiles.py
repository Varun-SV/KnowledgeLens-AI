...
LEGACY_PROFILES = (
    ProviderProfile("legacy-ollama", "Ollama / local", "ollama", "http://localhost:11434", "llama3.1", ("text",), is_builtin=True),
    ProviderProfile("legacy-llamacpp", "llama.cpp / local", "llama.cpp", "http://localhost:8080", "llama3.1", ("text",), is_builtin=True),
    ProviderProfile("legacy-openai", "OpenAI", "openai", "https://api.openai.com", "gpt-4o-mini", ("text", "vision", "structured_output"), is_builtin=True),
)
...
class ProviderProfileRepository:
        ...
        for profile in LEGACY_PROFILES:
            ...
            with self.database.connect() as connection:
                with connection.transaction():
                    connection.execute(
                        """
                        INSERT INTO provider_profiles(id, name, provider_type, base_url, default_model, capabilities, is_builtin, enabled)
                        VALUES (%s, %s, %s, %s, %s, %s::jsonb, TRUE, TRUE)
                        ON CONFLICT (name) DO NOTHING
                        """,
                        (profile_id, profile.name, profile.provider_type, profile.base_url, profile.default_model, json.dumps(profile.capabilities)),
                    )
    ...
    def save(self, *, name: str, provider_type: str, base_url: str, default_model: str, capabilities: Iterable[str], secret_ref: str | None = None, profile_id: str | None = None, is_builtin: bool = False) -> ProviderProfile:
        ...
        if not clean_model or len(clean_model) > 240:
            raise ValueError("Default model must be between 1 and 240 characters.")
        ...
        with self.database.connect() as connection:
            with connection.transaction():
                connection.execute(
                    """
                    INSERT INTO provider_profiles(id, name, provider_type, base_url, default_model, capabilities, secret_ref, is_builtin, enabled)
                    VALUES (%s, %s, %s, %s, %s, %s::jsonb, %s, %s, TRUE)
                    ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, provider_type = EXCLUDED.provider_type,
                      base_url = EXCLUDED.base_url, default_model = EXCLUDED.default_model, capabilities = EXCLUDED.capabilities,
                      secret_ref = EXCLUDED.secret_ref, updated_at = NOW()
                    """,
                    (profile_uuid, clean_name, clean_type, clean_url, clean_model, json.dumps(normalized_caps), secret_ref, is_builtin),
                )
        return ProviderProfile(str(profile_uuid), clean_name, clean_type, clean_url, clean_model, normalized_caps, secret_ref, is_builtin, True)
...
def _profile_from_row(row) -> ProviderProfile:
    ...
    return ProviderProfile(str(row["id"]), str(row["name"]), str(row["provider_type"]), str(row["base_url"]), str(row["default_model"]), tuple(str(item) for item in (capabilities or ["text"])), str(row["secret_ref"]) if row["secret_ref"] else None, bool(row["is_builtin"]), bool(row["enabled"]))
